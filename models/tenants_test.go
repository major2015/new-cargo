...
var (
	tenantDBTypes = map[string]string{`ID`: `uuid`, `Identifier`: `text`, `Name`: `text`, `SubscriptionID`: `integer`, `LogoURL`: `text`, `HomepageURL`: `text`, `Email`: `text`, `Metadata`: `jsonb`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_             = bytes.MinRead
)
